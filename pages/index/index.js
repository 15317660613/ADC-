...
Page({
  ...
  goCreateDaily: function(e) {
    ...
    let type = e.currentTarget.dataset.type;
     ...
     if(e.currentTarget.id){
       wx.navigateTo({
         url: 'createDaily?type=' + type + '&date=' + date + '&scheduleHourId=' + e.currentTarget.id,
       })
     }else{
       wx.navigateTo({
         url: 'createDaily?type=' + type + '&date=' + date,
       })
     }
    _this.setData({
      _lastTime: new Date()
  ...
  getOpenId() {
    console.log('urls.openidUrl','index.js')
    const _this = this;
    wx.login({
      // 获取code凭证
      success(res) {
  ...
  checkUserExist(openId) {
      console.log(' checkUserExist(openId) {','index.js')
    let _this = this;
    wx.request({
